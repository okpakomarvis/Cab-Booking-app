...
export const createDrivers = async (arg) => {
    ...
    let createdDrivers;
    try {
        const existingDrivers = await Drivers.findOne({ email: arg.DriverInput.email });
        if (existingDrivers){
            throw new Error(errorName.DRIVERA);
        }
        const result = await drivers.save();
        createdDrivers = { ...result._doc, _id: result.id };
...
export const createdriverLocation = async (arg) => {
    ...
    try {
        const existingDriversloc = await Driversloc.findOne({ driver_ID: arg.DriverlocInput.driver_ID });
        if (existingDriversloc) {
            throw new Error(errorName.DRIVERLOCALREADY);
        }
        const result = await driversloc.save();
        createdDriversloc = { ...result._doc, _id: result.id };
