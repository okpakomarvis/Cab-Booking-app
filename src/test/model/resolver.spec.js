...
const userData = {email:"best2559@gmail.com", password:"best"};
const driverData = {
        first_name: "Adams Ali",
        last_name: "Bacon",
        email: "bacon346@trec.com",
        gender: "Male",
        dob: "9/03/1989",
...
it('create & save Drivers location successfully', async () => {
  ...
  const savedDriverloc = await validDriver.save();
  // Object Id should be defined when successfully saved to MongoDB.
...
it('get all user past bookings', async() =>{
  const bookings = await Booking.find({ creator: { $in: creatorId } }).populate('creator');
  ...
  let maxDistance = 500; // request nearby car max distance 500 km
    ...
    const driverLocation = await DriverLoc.find();
        ...
        for (let i in driverLocation) {
          if (
              distance(userLatitude, userLongitude, driverLocation[i].latitude, driverLocation[i].longitude, 'K') <
              maxDistance
          ) {
